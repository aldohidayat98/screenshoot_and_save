...
def loop (ip1:(String), ip2:(String), lokasi:(String), nama:(String)):
    ...
    element.screenshot(lokasi+str(date)+'-2022_'+nama+'-WAN-IPSEC1_traffic.png')
   

def loop2 (ip1:(String), ip2:(String), lokasi:(String), nama:(String)):
    web = 'http://10.38.3.25/report.jsp?templid=_if&output=chart&device=' + ip1 + '&if=' + ip2 + '&chartTitle=Traffic+Rate'
    driver.get(web+str(epoch))
    driver.set_window_size(782, 768)
    time.sleep(1)
    element = driver.find_element('name','rep_form')
    element.screenshot(lokasi+str(date)+'-2022_'+nama+'-WAN-IPSEC2_traffic.png')
